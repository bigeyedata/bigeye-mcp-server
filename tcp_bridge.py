#!/usr/bin/env python3
"""
TCP bridge for Bigeye MCP server.
        ...
        try:
            while True:
                ...
                try:
                    ...
                    logger.debug(f"Client -> Server: {msg.get('method', msg.get('id', 'unknown'))}")
                except:
                    pass
    ...
    async def _server_to_client(self, server_stdout, writer: asyncio.StreamWriter):
        ...
        try:
            while True:
                ...
                try:
                    msg = json.loads(data.decode().strip())
                    logger.debug(f"Server -> Client: {msg.get('method', msg.get('id', 'unknown'))}")
                except:
                    pass
...
async def main():
    ...
    port = int(os.getenv("TCP_PORT", "5002"))
